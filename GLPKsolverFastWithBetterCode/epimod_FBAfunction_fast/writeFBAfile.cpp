 ...
 * the stoichiometry matrix, objective coefficients, and variable bounds, all formatted as specified
 * for easy parsing and use in optimization tasks.
 *
 ...
 *
 * Example:
 * writeModelCpp(S, react_id, obj_coef, lowbnd, uppbnd, rb, "example_model", true, "/my/directory");
 */
 // [[Rcpp::export]]
void writeModelCpp(NumericMatrix S, CharacterVector react_id, NumericVector obj_coef, NumericVector lowbnd, NumericVector uppbnd, NumericVector rb, string model_name, bool write, string wd) {
    if (!write) return;
    ...
    string fileName = wd + "/data/" + model_name + ".txt";
