...
fbamodel = readMATmod("models/icdf834.mat")

fbamodel@met_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["smet"]]
fbamodel@react_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["sreact"]]
fbamodel@met_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["dem"]]
fbamodel@react_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["der"]]
names <- readxl::read_excel("models/names.xlsx", col_names = FALSE)
fbamodel@react_name = as.character(c(1:fbamodel@react_num))
for (i in 1:length(names$...1)){
  fbamodel@react_name[i] = as.character(names[i, ]);
}

rxnGeneMat = readxl::read_excel("models/rxnGeneMat.xlsx", col_names = FALSE)

fbamodel@rxnGeneMat = Matrix::Matrix(0,
                                     nrow = fbamodel@react_num,
                                     ncol = length(fbamodel@allGenes),
                                     sparse = TRUE)

save(fbamodel, file = "models/icdf834.RData")

######################################################################################

fbamodel = readMATmod("models/iCN900.mat")

fbamodel@met_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["smet"]]
fbamodel@react_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["sreact"]]
fbamodel@met_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["dem"]]
fbamodel@react_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["der"]]
names <- readxl::read_excel("models/names_iCN900.xlsx", col_names = FALSE)
fbamodel@react_name = as.character(c(1:fbamodel@react_num))
...
rxnGeneMat = readxl::read_excel("models/rxnGeneMat_iCN900.xlsx", col_names = FALSE)

# rxnGeneMat: Object of class "matrix" containing a reaction to gene mapping.
# Matrix with rows corresponding to reactions and columns corresponding to genes.
...
sybil::modelorg2tsv(fbamodel, "BIGGdata_iCN900", extMetFlag = "b", fielddelim = "\t", 
                    entrydelim = ", ", makeClosedNetwork = FALSE, 
                    onlyReactionList = FALSE, minimalSet = TRUE,
                    fpath = "./models")

save(fbamodel, file = "models/iCN900.RData")

##########################################################################################

fbamodel = readMATmod("models/icdf838.mat")

fbamodel@met_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["smet"]]
fbamodel@react_single <- sybil::singletonMetabolites(fbamodel, retIds = FALSE)[["sreact"]]
fbamodel@met_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["dem"]]
fbamodel@react_de <- sybil::deadEndMetabolites(fbamodel, retIds = FALSE)[["der"]]
names <- readxl::read_excel("models/names_icdf838.xlsx", col_names = FALSE)
fbamodel@react_name = as.character(c(1:fbamodel@react_num))
for (i in 1:length(names$...1)){
  fbamodel@react_name[i] = as.character(names[i, ]);
}

rxnGeneMat = readxl::read_excel("models/rxnGeneMat_icdf838.xlsx", col_names = FALSE)
...
save(fbamodel, file = "models/icdf838.RData")
...
# findReactEq(Matlab.file, reagent = "feroxred[c]")
