...
modelProcessing <- function(path, model.name) {
  ...
  R.matlab::evaluate(matlab, paste("cd /home/riccardo/Documents/FBAandPN/data/", 
                                   model.name, sep = ""))
  ...
  model.mat = readMATmod(path)
  save(model.mat, file = paste("./data/", model.name, "/", model.name, ".RData", sep = ""))
  
  rm(model.mat) 
  
  # To load the data again
  load(file = paste("./data/", model.name, "/", model.name, ".RData", sep = ""))
  
  rxnGeneMat <- R.matlab::readMat(paste("./data/", model.name, "/", 
                                      "rxnGeneMat.mat", sep = ""))
  ...
  write.table(BiGGdata, file = paste("data/", 
                                     model.mat@mod_desc, "/BIGGdata_", 
                                     model.mat@mod_desc, "_react.tsv", 
                                     sep = ""), 
